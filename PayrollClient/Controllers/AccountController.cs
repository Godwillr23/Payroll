...
namespace PayrollClient.Controllers
{
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(ClientDetail model)
        {
            if (model.Email != null && model.LogPassword != null)
            {
                var v = _context.ClientDetails.Where(a => a.Email == model.Email).FirstOrDefault();
                if (v != null)
                {
                    if (string.Compare(Crypto.Hash(model.LogPassword), v.LogPassword) == 0)
                    {
                        ...
                        if (IsUserLegit != null)
                        {
                            ...
                            int userid = (from e in _context.ClientDetails
                                          where e.Email == email
                                          select e.ClientID).SingleOrDefault();
        ...
        public ActionResult UserProfile()
        {
            ...
            var user = _context.ClientDetails.FirstOrDefault(s => s.Email == userEmail);
            if (user == null)
            {
                return HttpNotFound();
            }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateProfile(ClientDetail userToUpdate)
        {
            if (ModelState.IsValid)
            {
                ...
                userToUpdate = _context.ClientDetails.FirstOrDefault(s => s.Email == userEmail);
        ...
        [AllowAnonymous]
        public ActionResult UpdatePassword(ClientDetail model)
        {
